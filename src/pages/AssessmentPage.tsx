							...
							{currentStep < steps.length - 1 ? (
								...
								<Button onClick={async () => {
									try {
										...
										// Parse result safely (function returns { exerciseProgram, isFallback, persisted })
										let result: any = null;
										try {
											result = typeof fnData === 'string' ? JSON.parse(fnData) : fnData;
										} catch (e) {
											result = fnData;
										}

										const exerciseProgram = result?.exerciseProgram || result?.exercise_program || result || null;
										// Show generated program in the UI
										setGeneratedProgram(exerciseProgram);

										// If the Edge Function didn't persist the recommendation, persist client-side (so Programs page shows it)
										let persisted = !!result?.persisted;
										if (!persisted) {
											try {
												const { error: pErr } = await supabase.from('recommendations').insert({
													assessment_id: inserted.id,
													program: exerciseProgram,
													confidence: result?.isFallback ? 0.3 : 0.8,
													source: result?.isFallback ? 'fallback' : 'ai'
												});
												if (pErr) {
													console.error('Client: failed to persist recommendation:', pErr);
												} else {
													persisted = true;
												}
											} catch (e) {
												console.error('Client persistence error:', e);
											}
										}

										// Inform the user depending on whether AI or fallback was used
										if (result?.isFallback) {
											toast({ title: 'AI unavailable', description: result?.error || 'Using fallback exercise program; saved to your programs.', variant: 'warning' });
										} else {
